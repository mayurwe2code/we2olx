...
async function resolveAndTestFacts(ecosystem, scans, options) {
    try {
        ...
        if (unauthorized) {
            throw errors_1.AuthFailedError('Unauthorized request to unmanaged service', error.code);
        }
        throw error;
    }
}
...
async function getDepsFromPlugin(root, options) {
    if (Object.keys(multiProjectProcessors).some((key) => options[key])) {
        ...
        const inspectRes = await multiProjectProcessors[scanType].handler(root, options, targetFiles);
    ...
    const inspectRes = await get_single_plugin_result_1.getSinglePluginResult(root, options);
...
async function processYarnWorkspaces(root, settings, targetFiles) {
    ...
    for (const directory of Object.keys(yarnTargetFiles)) {
        ...
        try {
            ...
            const yarnLock = get_file_contents_1.getFileContents(root, rootYarnLockfileName);
...
async function getCodeTestResults(root, options, sastSettings, requestId) {
    ...
    let baseURL = code_config_1.getCodeClientProxyUrl();
    const isLocalCodeEngineEnabled = localCodeEngine_1.isLocalCodeEngine(sastSettings);
    if (isLocalCodeEngineEnabled) {
        baseURL = sastSettings.localCodeEngine.url;
        if (options.debug) {
            await localCodeEngine_1.logLocalCodeEngineVersion(baseURL);
        }
    }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CodeClientError = void 0;
const custom_error_1 = __webpack_require__(17188);
...
function getCodeDisplayedOutput(args) {
    ...
    if (sarif.runs[0].results) {
        ...
        const results = args.shouldFilterIgnored
            ? utils_1.filterIgnoredIssues(sarif.runs[0].results)
    ...
    async test(paths, options) {
        ...
        try {
            ...
            const foundIssues = hasIgnoredIssues
                ? utils_1.filterIgnoredIssues(sarifRunResults)
                : sarifRunResults;
            ...
            const stringifiedResult = shouldStringifyResult
                ? json_1.jsonStringifyLargeObject(sarifTypedResult)
                : '';
            let sarifResult;
...
async function logLocalCodeEngineVersion(lceUrl = '') {
    ...
    try {
        const { res: { body, statusCode }, } = await request_1.makeRequest({
        ...
        while (step < common_1.RETRY_ATTEMPTS) {
            ...
            try {
                ...
                if (err instanceof errors_1.InternalServerError ||
                    ...
                    await common_2.sleep(common_1.RETRY_DELAY);
                }
                else {
                    break;
                }
